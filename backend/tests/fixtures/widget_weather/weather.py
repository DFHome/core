"""Weather API for the dashboard weather widget."""
import asyncio
import logging
import time
...
async def _fetch_wttr(client: httpx.AsyncClient, query: str) -> dict:
    resp = await client.get(
        f"https://wttr.in/{quote(query)}", params={"format": "j1", "lang": "ru"}
    )
    data = resp.json()
    ...
    for day in (data.get("weather") or [])[:2]:
        ...
        for slot in day.get("hourly") or []:
            try:
                t = datetime.strptime(
                    f"{date} {int(slot.get('time', 0)) // 100:02d}:00",
                    "%Y-%m-%d %H:%M",
                )
            except (TypeError, ValueError):
                continue
            ...
            if hour_floor and not (
                hour_floor <= t <= hour_floor + timedelta(hours=24)
            ):
                continue
            hourly_out.append(
                {
...
async def get_weather(query: str = Query(..., min_length=1)) -> dict:
    ...
    for name in sorted(_PROVIDERS, key=lambda n: n != _preferred):
        ...
        try:
            async with asyncio.timeout(deadline):
                async with _client(
                    timeout,
                    proxy=(settings.weather_proxy or None) if use_proxy else None,
                ) as client:
    ...
    if cached:
        return cached[1]
    raise HTTPException(
        status_code=502, detail="Не удалось получить погоду — " + "; ".join(errors)
    )
